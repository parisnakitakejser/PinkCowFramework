...
class Util {
	...
	public static function loadTpl( $file = '' ) {
		if ( !defined('PINKCOW_UTIL_TEMPLATE_PATH') ) {
			$content = _('You need to define PINKCOW_UTIL_TEMPLATE_PATH in your config file.');
		} else {
			...
			if ( file_exists( $url . $file ) ) {
				...
				$content = _('The file you are trying to load does not exist<br /><i>Path: '. $url . $file .'</i>' );
			}
		}
